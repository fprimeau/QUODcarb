...
% 
% OUT_ac = zeros(102,83); err_ac = zeros(102,18);
% OUT_cp = zeros(102,83); err_cp = zeros(102,18);
% OUT_ap = zeros(102,83); err_ap = zeros(102,18);

%     [OUT_ac1,err_ac1,OUT_cp1,err_cp1,OUT_ap1,err_ap1] = compare(yobs,wobs,temp(i),sal(i),pres(i),sys);
%     %keyboard
%     OUT_ac(i,:) = OUT_ac1; err_ac(i,:) = err_ac1;
%     OUT_cp(i,:) = OUT_cp1; err_cp(i,:) = err_cp1;
%     OUT_ap(i,:) = OUT_ap1; err_ap(i,:) = err_ap1;


function [A] = compare(obs,pair,est)
    ...
    % exit struct
    out.TA = OUT(1)*1e-6; % mol/kg
    out.wTA = (err(1)*1e-6)^(-1/2); % sigma, not precision
    out.TC = OUT(2)*1e-6;
    out.wTC = (err(2)*1e-6)^(-1/2);
    out.ph = OUT(37); % total scale
    out.wph = (p(err(10)*1e-9))^(-1/2); % was nano units
    out.pco2 = OUT(19)*1e-6;
    out.wpco2 = (err(11)*1e-6)^(-1/2);
    out.pK0 = p(OUT(66));
    out.wpK0 = (0.002)^(-1/2);
    out.pK1 = p(OUT(67));
    out.wpK1 = (0.01)^(-1/2);
    ...
    A = [out.TA, out.wTA, est.TA, est.eTA, ...
         out.TC, out.wTC, est.TC, est.eTC, ...
         out.ph, out.wph, est.m(1).ph, est.m(1).eph, ...
         out.pco2, out.wpco2, est.m(1).pco2, est.m(1).epco2, ...
         out.pK0, out.wpK0, est.m(1).pK0, est.m(1).epK0, ...
         out.pK1, out.wpK1, est.m(1).pK1, est.m(1).epK1, ...
         out.pK2, out.wpK2, est.m(1).pK2, est.m(1).epK2, ...
         out.pKw, out.wpKw, est.m(1).pKw, est.m(1).epKw, ...
         out.pKb, out.wpKb, est.m(1).pKb, est.m(1).epKb, ...
         out.pKs, est.m(1).pKs, ...
         out.pKf, est.m(1).pKf, ...
         out.pK1p, est.m(1).pK1p, ...
         out.pK2p, est.m(1).pK2p, ...
         out.pK3p, est.m(1).pK3p, ...
         out.pKSi, est.m(1).pKsi];
