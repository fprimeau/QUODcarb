...
% utility functions and constants
    ...
    for j = 1:nTP
        ...
        if phscale == 2 || phscale == 4 % sws or nbs
            gpS2f_outside = @(z) dpdx( 1 + ( q( z(iTS) ) / ...
                q( z(m(j).iKs) ) ) + ( q( z(iTF) ) / q( z(m(j).iKf) ) ) );
            gpS2f_pTS = @(z) gpS2f_outside(z) * ...
                ( dqdx(z(iTS) ) / q( z(m(j).iKs) ) );
            gpS2f_pKs = @(z) gpS2f_outside(z) * ...
                ( q( z(iTS) ) / dqdx( z(m(j).iKs) ) )  ...
                * ( -1 * ( q( z(m(j).iKs) ) )^(-2) );
            gpS2f_pTF = @(z) gpS2f_outside(z) ...
                * ( dqdx( z(iTF)) / q( z(m(j).iKf) ) );
            gpS2f_pKf = @(z) gpS2f_outside(z) ...
                * ( q( z(iTF) ) / dqdx( z(m(j).iKf) ) )  ...
                * (-1 * ( q( z(m(j).iKf) ) )^(-2) );
            if phscale == 2 % sws
                g_pfH = @(z) 0;
                m(j).grph_free = @(z) [ gpS2f_pTS(z); gpS2f_pKs(z); ...
                    gpS2f_pTF(z); gpS2f_pKf(z); ...
                    g_pfH(z); 1              ; -1];
