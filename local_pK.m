...
%   gpK  = [pK_T, pK_S, pK_P]; first derivatives (gradient of pK)
    ...
    function [pKb,gpKb] = calc_pKb(TC, TK,S,Pbar,RT, RT_T)
        ...
        pKb_T = -(-lnKbt./(TK.^2) + (1./TK).*(n + o.*sqrt(S) + r.*S) + ...
    ...
    function [pKw,gpKw] = calc_pKw(TC,TK,S,Pbar,RT,RT_T)
        ...
        %    sqrt(S) - 0.01615 .* S;
        pKw = -( a + b/TK + c.*log(TK) + ( d + g./TK + h.*log(TK) ) .* ...
            sqrt(S) + l.*S ) ./ log(10);
        ...
        % pressure correction
        aa = -20.02; bb = 0.1119; cc = -001409; dd = -5.13; 
        gg = 0.0794; hh = 1000;
    ...
    function [pK1p,gpK1p] = calc_pK1p(TC,TK,S,Pbar,RT,RT_T)
        ...
        pK1p = -( a./TK + b + c.*TK + (d./TK + g) .* sqrt(S) + ...
                (h./TK + l).*S ) ./ log(10);
        pK1p_T = - ( -a./(TK.^2) + c + sqrt(S) .* (-d./(TK.^2)) + ...
   ...
   % S and P are fine but T is NOT
    ...
    [pK1,gpK1] = calc_pK1(TC,TK,SPbar,RT,RT_T);
